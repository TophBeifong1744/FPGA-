...
- `AXI BRAM Controller`：32-bit data width，Single Port BRAM interface。
- `Block Memory Generator`：True Dual Port RAM，Native interface，32-bit width，depth 64。
- Port A 接 AXI BRAM Controller；Port B 由一个“feature_writer”状态机在 `frame_done` 后写入。
- 本代码包已给出 `rtl/feature_bram_writer.v`；其 `bram_*` 端口接 BMG Port B 的 `enb/web/addrb/dinb`，`clkb` 接同一 `FCLK_CLK0`。
