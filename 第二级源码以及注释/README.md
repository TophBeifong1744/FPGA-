...
> 本注释修订版的模块连接、时钟复位、BRAM字节地址和管理原理，统一见
> [`MODULE_CONNECTION_GUIDE.md`](MODULE_CONNECTION_GUIDE.md)。其中
> `running_stats16.v` 已采用结算流水线，`feature_bram_writer.v` 已改为
> 32位BRAM字节地址，以对应Vivado的BRAM Controller接口。
...
| 8～23 | `0x20～0x5C` | `HIST_MAG[0..15]` |
| 24～39 | `0x60～0x9C` | `HIST_FREQ[0..15]` |
...
其中 `Fs` 是第一级抽取后的实际有效采样率，不是 100 MHz PL 时钟。
...
| `sim/tb_running_stats16.v` | 流水线均值/方差模块独立测试 |
